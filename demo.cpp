...
int main() {
    ...
    int64_t b = dis(gen);
    int64_t c = dis(gen);
    int64_t n = ProduceRandomPrime();
    int64_t g = PrimitiveElement(n);
    cout << "大素数：\t" << n << endl;
    cout << "原  根：\t" << g << endl;
    cout << "随机数a：\t" << a << endl;
    cout << "随机数b：\t" << b << endl;
    cout << "随机数c：\t" << c << endl;
    cout << "---------计算---------"<< endl;
    int64_t Sa = mod_pow(g,a,n);
    cout << "Sa = g^a(modn) =     " << Sa << endl;
    int64_t Sab = mod_pow(Sa,b,n);
    cout << "Sab = Sa^b(modn) =   " << Sab << endl;
    int64_t Sabc = mod_pow(Sab,c,n);
    cout << "Sabc = Sab^c(modn) = " << Sabc << "(协商密钥)";
